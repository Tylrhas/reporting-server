...
function non_associated_total(firstDay, lastDay) {
  ...
  firstDay = dates.pst_to_utc(firstDay)
  lastDay = dates.pst_to_utc(lastDay)
...
async function current_backlog(firstDay, lastDay) {
  ...
  if (dates.moment(lastDay).isAfter(date)) {
    if (dates.moment(firstDay).isBefore(date)) {
      firstDay = dates.moment(date).format('MM-DD-YYYY')
    }
    // display the backlog
    firstDay = dates.pst_to_utc(firstDay)
    lastDay = dates.pst_to_utc(lastDay)
